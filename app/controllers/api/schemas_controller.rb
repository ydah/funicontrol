module Api
  class SchemasController < ApplicationController
    ...
    SCHEMAS = {
      "line" => {
        attributes: ATTRIBUTE_SCHEMA.call(%w[id name slug status weather_condition passenger_satisfaction_score description stations cars track_segments open_incidents_count running_cars_count critical_incidents_count recent_events created_at updated_at]),
        endpoints: {
          all: { path: "/api/lines" },
      ...
      "incident" => {
        attributes: ATTRIBUTE_SCHEMA.call(%w[id line_id station_id car_id kind severity status title description photo_url photo_filename attachments open_seconds sla_status resolved_at station car incident_comments related_events created_at updated_at]),
        endpoints: {
